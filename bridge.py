...
2. Si un zombie est a dist < 3 → attack_nearest si arme equipee, sinon move_to (fuir)
3. Si hunger > 0.6 et nourriture en inventaire → eat_best_food
4. Si pas d'arme equipee et HandWeapon en inventaire → equip_best_weapon
5. Si conteneur non explore a moins de 5 tiles → loot_container
6. Sinon explorer (move_to vers position inconnue)

REGLES :
- Reponds UNIQUEMENT avec le JSON de l'action, rien d'autre, pas de markdown
- Ne jamais attaquer sans arme equipee
...
def format_obs_summary(obs: dict) -> str:
    ...
    primary = equip.get("primary", {}).get("name", "—")
    ...
    return (
        ...
        f"weapon={primary} | "
        f"inv={len(inv)} items ({food_count} food, {weapon_count} weapons) | "
    ...
    while True:
        try:
            ...
            if inv:
                ...
                if weapons:
                    print(f"[bridge] WEAP | " + ", ".join(i['name'] for i in weapons))
                if other:
